                ...
                return (
                    <div key={index} className='tasklist-container border-red flex gap-2 '>
                        <div className='tasklist-checkbox'>
                            <input type="checkbox" aria-label={`Mark task ${task.title} as ${task.checked ? 'pending' : 'completed'}`} checked={task.checked || false} onChange={() => handleCheck(index)} />
                        </div>
                        <div className="tasklist-card">
                            ...
                            <div className="tasklist-card-body flex gap-5 text-[13px] items-center">
                                ...
                                <span className='tm-avatar avatar-rv' aria-hidden="true">{task.assignee.substring(0, 2).toUpperCase()}</span>
                        ...
                        <div className="tasklist-card-btn">
                            <button aria-label={`Delete task ${task.title}`} onClick={() => handleDelete(index)}>Delete</button>
                        </div>
                    </div>
                )
